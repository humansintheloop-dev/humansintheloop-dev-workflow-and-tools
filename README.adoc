...
* The `i2c` CLI tool — a UV-installable Python CLI for plan file management (reading, updating, and structuring implementation plans)
...
The `i2c` CLI tool provides structured plan file management commands used by the `plan-file-management` skill to enable Claude to efficiently read and edit plan files.
It supports marking tasks and steps complete, inserting/deleting/reordering threads and tasks, and querying plan status.

Example commands:

[source,bash]
----
i2c plan get-next-task path/to/plan.md
i2c plan mark-task-complete path/to/plan.md --thread 1 --task 2 --rationale "All tests passing"
i2c plan list-threads path/to/plan.md
----
...
* `plan-file-management` - tells Claude when and how to use `i2c plan` commands to read and edit plan files
