...
* Supporting scripts.
* The `i2code` CLI tool — a UV-installable Python CLI that's the future home of the other scripts. 
...
In addition, there are the following subcommands:

* `i2code improve review-issues` - analyzes filed issues and updates the Claude Code plugin
* `i2code improve update-claude-files` - updates the template files with changes made to a project's `CLAUDE.md` and `.settings.local.json`
* `i2code setup update-project` - updates a project's `CLAUDE.md` and `.settings.local.json` from the template files.
