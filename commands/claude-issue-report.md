...
6. Create a report in the project root's `.hitl/issues/active/` directory using an absolute path. IMPORTANT: Use the project root directory (where you started), not the current working directory if you have changed directories during the session. The file path must be absolute (e.g., `/Users/name/project/.hitl/issues/active/YYYY-MM-DD-HH-MM-SS.md`). Format:
