...
def cmd_mark_step_incomplete(args):
    """Handle the mark-step-incomplete subcommand."""
    with open(args.plan_file, 'r', encoding='utf-8') as f:
        plan = f.read()

    try:
        result = mark_step_incomplete(plan, args.thread, args.task, args.step, args.rationale)
    except ValueError as e:
        print(str(e), file=sys.stderr)
        sys.exit(1)

    atomic_write(args.plan_file, result)
    print(f"Marked step {args.step} of task {args.thread}.{args.task} as incomplete")
...
def build_parser():
    ...
    mark_step_inc = subparsers.add_parser('mark-step-incomplete',
                                           help='Mark a completed step as incomplete')
    mark_step_inc.add_argument('plan_file', help='Path to the plan file')
    mark_step_inc.add_argument('--thread', type=int, required=True, help='Thread number')
    mark_step_inc.add_argument('--task', type=int, required=True, help='Task number')
    mark_step_inc.add_argument('--step', type=int, required=True, help='Step number')
    mark_step_inc.add_argument('--rationale', required=True, help='Reason for the change')
    mark_step_inc.set_defaults(func=cmd_mark_step_incomplete)
