#!/usr/bin/env bash
# Verify that all pytest tests have either @pytest.mark.unit or @pytest.mark.integration markers.
set -euo pipefail
...
UNMARKED=$(uv run --python 3.12 --with pytest --with GitPython --with pytest-mock --with Jinja2 \
    python3 -m pytest "$PROJECT_ROOT/tests/" --co -q -m "not unit and not integration" 2>&1) || true
