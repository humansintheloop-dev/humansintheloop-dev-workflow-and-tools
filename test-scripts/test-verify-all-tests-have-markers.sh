#!/usr/bin/env bash
# Verify that all pytest tests have a recognized marker (unit, integration, integration_gh, or manual).
set -euo pipefail
...
UNMARKED=$(uv run --python 3.12 --with pytest --with GitPython --with pytest-mock --with Jinja2 \
    python3 -m pytest "$PROJECT_ROOT/tests/" --co -q -m "not unit and not integration and not integration_gh and not integration_claude and not manual" 2>&1) || true
