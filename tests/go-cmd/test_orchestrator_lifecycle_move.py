...
def _fake_transition(name, old_path, new_state, git_root):
    """Simulate transition by updating the metadata file."""
    metadata_path = os.path.join(str(old_path), f"{name}-metadata.yaml")
    with open(metadata_path, "w") as f:
...
class TestMoveMenuHandler:
    ...
    def test_move_keeps_project_in_same_directory(self):
        with _run_move_draft_to_ready() as (orch, _, _displayed):
            assert "/active/" in orch._project.directory
