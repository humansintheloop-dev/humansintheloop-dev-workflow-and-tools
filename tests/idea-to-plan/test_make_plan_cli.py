...
class TestMakePlanInvokesBundledScript:
    def test_invokes_make_plan_sh_with_directory_arg(self):
        runner = CliRunner()
        with patch("i2code.script_runner.subprocess") as mock_subprocess:
            ...
            result = runner.invoke(main, ["idea-to-plan", "make-plan", "my-dir"])

            assert result.exit_code == 0
            mock_subprocess.run.assert_called_once()
            call_args = mock_subprocess.run.call_args[0][0]
            assert Path(call_args[0]) == SCRIPTS_DIR / "make-plan.sh"
