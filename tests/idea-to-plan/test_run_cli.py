...
class TestRunInvokesBundledScript:
    def test_invokes_idea_to_code_sh_with_directory_arg(self):
        runner = CliRunner()
        with patch("i2code.script_runner.subprocess") as mock_subprocess:
            ...
            result = runner.invoke(main, ["idea-to-plan", "run", "my-dir"])

            assert result.exit_code == 0
            mock_subprocess.run.assert_called_once()
            call_args = mock_subprocess.run.call_args[0][0]
            assert Path(call_args[0]) == SCRIPTS_DIR / "idea-to-code.sh"
    ...
    def test_forwards_multiple_arguments(self):
        ...
        with patch("i2code.script_runner.subprocess") as mock_subprocess:
            ...
            result = runner.invoke(
                main, ["idea-to-plan", "run", "my-dir", "--verbose", "extra"]
            )

            assert result.exit_code == 0
            call_args = mock_subprocess.run.call_args[0][0]
            assert call_args[1:] == ["my-dir", "--verbose", "extra"]
