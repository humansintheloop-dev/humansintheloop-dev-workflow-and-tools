"""Tests for Plan.insert_thread_after() method."""
...
class TestPlanInsertThreadAfter:

    def test_inserts_after_first_thread(self):
        plan = parse(PLAN_TEXT)
        plan.insert_thread_after(1, _new_thread())
    ...
    def test_inserts_after_last_thread(self):
        ...
        assert len(plan.threads) == 3
        assert plan.threads[2].tasks[0].title == 'New task'

    def test_renumbers_in_output(self):
        plan = parse(PLAN_TEXT)
        plan.insert_thread_after(1, _new_thread())
        text = plan.to_text()
        assert "Steel Thread 1: First Thread" in text
        assert "Steel Thread 2: New Thread" in text
        assert "Steel Thread 3: Second Thread" in text
        assert "Task 1.1: First task" in text
        assert "Task 2.1: New task" in text
    ...
    def test_error_for_nonexistent_thread(self):
        ...
        with pytest.raises(ValueError, match="thread 99 does not exist"):
            plan.insert_thread_after(99, _new_thread())
