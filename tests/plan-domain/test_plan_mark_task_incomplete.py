"""Tests for Plan.mark_task_incomplete() delegation."""
...
class TestPlanMarkTaskIncomplete:
    """Plan.mark_task_incomplete() delegates to Task and validates inputs."""

    def test_marks_task_incomplete(self):
        plan = parse(PLAN_TEXT)
        plan.mark_task_incomplete(1, 1)
        assert not plan.threads[0].tasks[0].is_completed

    def test_does_not_affect_other_tasks(self):
        plan = parse(PLAN_TEXT)
        plan.mark_task_incomplete(1, 1)
        assert plan.threads[0].tasks[1].is_completed

    def test_error_for_nonexistent_thread(self):
        plan = parse(PLAN_TEXT)
        with pytest.raises(ValueError, match="thread 99 does not exist"):
            plan.mark_task_incomplete(99, 1)
