...
def _invoke_insert_after_with_task_file(plan_file, task_json=TASK_FILE_JSON, extra_args=None):
    """Write plan and task JSON file, invoke insert-task-after with --task-file."""
    plan_file.write_text(PLAN_WITH_TWO_TASKS)
...
class TestInsertTaskAfterCliWithTaskFile:
    """CLI integration: insert-task-after supports --task-file JSON input."""

    def test_inserts_using_task_file(self, tmp_path):
        result, updated = _invoke_insert_after_with_task_file(tmp_path / "plan.md")
        assert result.exit_code == 0
    ...
    def test_error_when_task_file_combined_with_individual_option(self, tmp_path):
        result, _ = _invoke_insert_after_with_task_file(
            tmp_path / "plan.md", extra_args=["--title", "Override"],
        )
        assert result.exit_code == 1
        assert "insert-task-after: --task-file and individual task options are mutually exclusive" in result.output
    ...
    def test_error_when_no_options_provided(self, tmp_path):
        ...
        result = CliRunner(catch_exceptions=False).invoke(insert_task_after_cmd, args)
        assert result.exit_code == 1
        assert "insert-task-after: either --task-file or all individual task options are required" in result.output
    ...
    def test_error_when_task_file_missing_required_field(self, tmp_path):
        ...
        assert result.exit_code == 1
        assert "insert-task-after: --task-file JSON is missing required field: evidence" in result.output
    ...
    def test_error_when_task_file_invalid_json(self, tmp_path):
        ...
        assert result.exit_code == 1
        assert "insert-task-after: --task-file is not valid JSON:" in result.output
