...
class TestPerFileShaReading:
    ...
    def test_reads_claude_md_previous_sha_from_marker(
        self, fake_runner, fake_renderer,
    ):
        with tempfile.TemporaryDirectory() as tmpdir:
            captured, claude_md_relpath, _ = _run_capturing_git_calls(
                tmpdir, (fake_runner, fake_renderer),
                markers={"claude_md": "AAA111", "settings": "BBB222"},
            )
            diff_calls = [
                c for c in captured
                if "diff" in " ".join(c) and c[-1] == claude_md_relpath
            ]
            assert len(diff_calls) == 1
            assert "AAA111..CCC333" in " ".join(diff_calls[0])

    def test_reads_settings_previous_sha_from_fake_permission_entry(
        self, fake_runner, fake_renderer,
    ):
        with tempfile.TemporaryDirectory() as tmpdir:
            captured, _, settings_relpath = _run_capturing_git_calls(
                tmpdir, (fake_runner, fake_renderer),
                markers={"claude_md": "AAA111", "settings": "BBB222"},
            )
            diff_calls = [
                c for c in captured
                if "diff" in " ".join(c) and c[-1] == settings_relpath
            ]
            assert len(diff_calls) == 1
            assert "BBB222..DDD444" in " ".join(diff_calls[0])
